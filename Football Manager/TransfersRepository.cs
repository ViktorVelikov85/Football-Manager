...
namespace Football_Manager
{
    ...
    {
        // Метод за добавяне на трансфер с ТРАНЗАКЦИЯ
        public void AddTransfer(int playerId, int? fromClubId, int toClubId, DateTime date, decimal fee)
        {
            using (var conn = new MySqlConnection(Db.connString))
            {
                conn.Open();
                using (var transaction = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        MySqlCommand cmd1 = new MySqlCommand(sqlTransfer, conn, transaction);
                        ...
                        // 2. Ъпдейт на текущия клуб на играча
                        string sqlUpdate = "UPDATE players SET club_id = @tId WHERE id = @pId";
                        MySqlCommand cmd2 = new MySqlCommand(sqlUpdate, conn, transaction);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        throw new Exception("Грешка при изпълнение на трансфера: " + ex.Message);
                    }
                }
            }
        }
